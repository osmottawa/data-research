...
source.results.collection1.map(result => {
  ...
  if (exception) {
    if (exception.street) { street = exception.street }
    if (exception.housenumber) { housenumber = exception.housenumber }
    if (exception.phone) { phone = exception.phone }
    if (exception.unit) { unit = exception.unit }
  }
